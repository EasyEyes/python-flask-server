...
def test_run():
    ...
    for i in range(num_captured):
        #MLS = readCSVData(f'/home/billy/Documents/EasyEyes/presentation/data/MLS.csv')
        #rec = readCSVData(f'/home/billy/Documents/EasyEyes/presentation/data/recordedMLSignal_0.csv')
        MLS = pd.read_csv('/home/billy/Downloads/MLS.csv').to_numpy().squeeze()
        rec = pd.read_csv('/home/billy/Downloads/recordedMLSignal_0.csv').to_numpy().squeeze()
        print(MLS)
        print(rec)
        ir, debug = run_ir_task(MLS,rec,debug=True) 
        print('here')
        saveBufferToCSV(ir.real,f'/home/billy/Documents/EasyEyes/presentation/data/ivan_ir_0.csv')
